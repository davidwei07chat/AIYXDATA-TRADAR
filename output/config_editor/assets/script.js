...
const REMOTE_FREQUENCY_URL = 'https://raw.githubusercontent.com/AIYXDATA/TrendRadar/refs/heads/master/config/frequency_words.txt';
const REMOTE_TIMELINE_URL = 'https://raw.githubusercontent.com/AIYXDATA/TrendRadar/refs/heads/master/config/timeline.yaml';
const REMOTE_VERSION_URL = 'https://raw.githubusercontent.com/AIYXDATA/TrendRadar/refs/heads/master/version_configs';
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 420px;">
            ...
            <div class="text-xs text-gray-400 mt-3 p-2 bg-gray-50 rounded">
                ...
                数据来源：<a href="https://github.com/AIYXDATA/TrendRadar" target="_blank" class="text-blue-500 hover:underline">AIYXDATA/TrendRadar</a>
            </div>
    ...
    switch(mod.key) {
        case "platforms":
            ...
            html += `<div class="flex items-center gap-2 mt-3">
                        ...
                        <a href="https://github.com/AIYXDATA/TrendRadar?tab=readme-ov-file#%E9%85%8D%E7%BD%AE%E8%AF%A6%E8%A7%A3" target="_blank" class="text-xs bg-gray-100 text-gray-600 px-3 py-1.5 rounded hover:bg-gray-200 transition-colors border border-gray-200 flex items-center gap-1 no-underline">
        ...
        case "ai":
            html = `
            <div class="mb-4">
                <label class="block text-xs font-bold text-gray-700 mb-2">快速选择预设模板 / Quick Presets</label>
                <div class="flex flex-wrap gap-2 mb-3" id="ai-presets-container">
                    <button type="button" class="px-3 py-1.5 text-xs border border-gray-300 rounded hover:bg-blue-50 hover:text-blue-600 hover:border-blue-300 transition-colors ai-preset-btn" data-model="Kimi/moonshot-v1-8k" data-base="https://api.moonshot.cn/v1">Kimi (Moonshot)</button>
                    <button type="button" class="px-3 py-1.5 text-xs border border-gray-300 rounded hover:bg-blue-50 hover:text-blue-600 hover:border-blue-300 transition-colors ai-preset-btn" data-model="zhipu/glm-4-flash" data-base="">智谱 (Zhipu AI)</button>
                    <button type="button" class="px-3 py-1.5 text-xs border border-gray-300 rounded hover:bg-blue-50 hover:text-blue-600 hover:border-blue-300 transition-colors ai-preset-btn" data-model="deepseek/deepseek-chat" data-base="">DeepSeek</button>
...
window.toggleGlobalLock = function() {
    ...
    if (isGlobalLocked) {
        btn.innerHTML = '<i class="fa-solid fa-lock"></i>锁定编辑';
        ...
        applyLockState();
        showToast('已锁定编辑', 'info');
    } else {
        btn.innerHTML = '<i class="fa-solid fa-unlock"></i>解除锁定';
...
window.reloadAllConfigsFromServer = function(silent = false) {
    ...
    Promise.all([
        ...
        if (configRes.success && configRes.content) {
            document.getElementById('yaml-editor').value = configRes.content;
        ...
        // 处理 frequency_words.txt
        if (frequencyRes.success && frequencyRes.content) {
            document.getElementById('frequency-editor').value = frequencyRes.content;
            currentFrequency = frequencyRes.content;
            currentFrequencyData = null;
            updateBackdrop('frequency-editor', 'frequency-backdrop');
            syncFrequencyToUI();
            localStorage.setItem(STORAGE_KEY_FREQUENCY, currentFrequency);
        ...
        // 处理 timeline.yaml
        if (timelineRes.success && timelineRes.content) {
            document.getElementById('timeline-editor').value = timelineRes.content;
...
// ==========================================
// 16. APPLY 应用同步到后端 (Apply to Backend)
// ==========================================

window.applyToBackend = function() {
    showToast('正在应用配置到服务器...', 'info');
    ...
    return Promise.all([
        fetch('/api/save', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ file: 'config', content: configContent })
        }).then(r => r.json()),
        fetch('/api/save', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ file: 'frequency', content: frequencyContent })
        }).then(r => r.json()),
        fetch('/api/save', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (!frequencyRes.success) {
            hasError = true;
            errorMsg += 'frequency_words.txt 保存失败; ';
        }
...
// --- AI Presets Logic ---
window.applyPreset = function(model, base) {
    const modelInput = document.querySelector('input[data-path="model"]');
    const baseInput = document.querySelector('input[data-path="api_base"]');
    
    if (modelInput) {
        modelInput.value = model;
        updateYamlFromUI('ai', 'model', modelInput);
    }
...
window.renderCustomPresets = function() {
    ...
    const addBtn = container.querySelector('button[onclick="addCustomPreset()"]');
    
    presets.forEach((preset, index) => {
        const btn = document.createElement('div');
        btn.className = 'custom-preset relative group inline-block';
        btn.innerHTML = `
            <button type="button" class="px-3 py-1.5 pr-6 text-xs border border-green-300 bg-green-50 text-green-700 rounded hover:bg-green-100 transition-colors" 
                    onclick="applyPreset('${preset.model}', '${preset.base}'); showToast('已应用自定义预设: ${preset.name}', 'success')">
                ${preset.name}
            </button>
            <button type="button" class="absolute right-1 top-1.5 text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100" 
                    onclick="deleteCustomPreset(${index})" title="删除预设">
...
window.saveCurrentAsCustomPreset = function() {
    const modelInput = document.querySelector('input[data-path="model"]');
    const baseInput = document.querySelector('input[data-path="api_base"]');
    ...
    const name = prompt("给这个自定义配置起个名字:", modelInput.value.split('/').pop() || "自定义配置");
    if (!name) return;
