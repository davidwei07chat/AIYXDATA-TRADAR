...
def show_status():
    ...
    if supercronic_is_pid1:
        ...
        print("    💡 建议操作:")
        print("       • 重启容器: docker restart aiyxdata_tradar")
        print("       • 检查容器日志: docker logs aiyxdata_tradar")

    # 显示日志检查建议
    print("  📋 运行状态检查:")
    print("    • 查看完整容器日志: docker logs aiyxdata_tradar")
    print("    • 查看实时日志: docker logs -f aiyxdata_tradar")
    print("    • 手动执行测试: python manage.py run")
    print("    • 重启容器服务: docker restart aiyxdata_tradar")
...
def show_logs():
    ...
    try:
        ...
        for log_file in log_files:
            ...
            print("📋 无法找到标准日志文件，建议使用: docker logs aiyxdata_tradar")
            
    except KeyboardInterrupt:
        print("\n👋 退出日志查看")
    except Exception as e:
        print(f"❌ 查看日志失败: {e}")
        print("💡 建议使用: docker logs aiyxdata_tradar")
...
def restart_supercronic():
    ...
    try:
        ...
        if "supercronic" in pid1_cmdline.lower():
            ...
            print("    docker restart aiyxdata_tradar")
        else:
        ...
        print("  💡 建议重启容器: docker restart aiyxdata_tradar")
...
def start_webserver():
            ...
            with open(WEBSERVER_PID_FILE, 'r') as f:
                ...
                if old_pid_str:
                    ...
                    try:
                        ...
                        # 进程不存在，删除旧的 PID 文件
                        print(f"  🧹 清理陈旧的 PID 文件 (PID: {old_pid})")
                        os.remove(WEBSERVER_PID_FILE)
                else:
                    os.remove(WEBSERVER_PID_FILE)
        except Exception as e:
            print(f"  ⚠️ 清理旧的 PID 文件失败: {e}")
            try:
                os.remove(WEBSERVER_PID_FILE)
            except:
                pass
    ...
    try:
        # 启动自定义 HTTP 服务器支持配置保存 (Start custom HTTP server supporting config saves)
        server_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'server.py')
        
        # 将日志重定向到文件以便排查 (Redirect logs to file for troubleshooting)
        log_file_path = os.path.join(WEBSERVER_DIR, 'webserver.log')
...
💡 常用操作指南:
  ...
  3. 查看日志: logs
     - 实时监控运行情况
     - 也可使用: docker logs aiyxdata_tradar

  4. 重启服务: restart
     - 由于 supercronic 是 PID 1，需要重启整个容器
     - 使用: docker restart aiyxdata_tradar
