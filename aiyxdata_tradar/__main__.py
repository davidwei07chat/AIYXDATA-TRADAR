...
from aiyxdata_tradar import __version__
from aiyxdata_tradar.core import load_config
from aiyxdata_tradar.core.analyzer import convert_keyword_stats_to_platform_stats, format_time_display
from aiyxdata_tradar.crawler import DataFetcher
from aiyxdata_tradar.storage import convert_crawl_results_to_news_data
from aiyxdata_tradar.utils.time import DEFAULT_TIMEZONE, is_within_days, calculate_days_old
from aiyxdata_tradar.ai import AIAnalyzer, AIAnalysisResult
from aiyxdata_tradar.core.scheduler import ResolvedSchedule
...
class NewsAnalyzer:
            ...
            for item in rss_items:
                ...
                if feed_id in rss_feed_ids:
                    ...
                    feed_items_map[feed_id]["items"].append({
                        ...
                        "time_display": item.get("published_at", ""),  # RSS uses published_at as display
    ...
    def _process_rss_data_by_mode(self, rss_data) -> Tuple[Optional[List[Dict]], Optional[List[Dict]], Optional[List[Dict]]]:
        ...
        Returns:
            ...
            - raw_rss_items: 原始 RSS 条目列表（用于独立展示区）
        """
        from aiyxdata_tradar.core.analyzer import count_rss_frequency
    ...
    def _filter_rss_by_keywords(self, rss_items: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            if word_groups or filter_words or global_filters:
                from aiyxdata_tradar.core.frequency import matches_word_groups
                filtered_items = []
                for item in rss_items:
                    title = item.get("title", "")
    ...
    def _generate_rss_html_report(self, rss_items: list, feeds_info: dict) -> str:
        """生成 RSS HTML 报告"""
        try:
            from aiyxdata_tradar.report.rss_html import render_rss_html_content
            from pathlib import Path
...
示例:
  python -m aiyxdata_tradar                    # 正常运行
  python -m aiyxdata_tradar --show-schedule    # 查看当前调度状态
"""
    )
    parser.add_argument(
        "--show-schedule",
