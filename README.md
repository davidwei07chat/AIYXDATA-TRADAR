...
[![RSS](https://img.shields.io/badge/RSS-订阅源支持-orange.svg?style=flat-square&logo=rss&logoColor=white)](https://github.com/AIYXDATA/AIYXDATA-TRADAR)
[![AI翻译](https://img.shields.io/badge/AI-多语言推送-purple.svg?style=flat-square)](https://github.com/AIYXDATA/AIYXDATA-TRADAR)
...
[![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-自动化-2088FF?style=flat-square&logo=github-actions&logoColor=white)](https://github.com/AIYXDATA/AIYXDATA-TRADAR)
[![GitHub Pages](https://img.shields.io/badge/GitHub_Pages-部署-4285F4?style=flat-square&logo=github&logoColor=white)](https://AIYXDATA.github.io/AIYXDATA-TRADAR)
[![Docker](https://img.shields.io/badge/Docker-部署-2496ED?style=flat-square&logo=docker&logoColor=white)](https://hub.docker.com/r/wantcat/aiyxdata_tradar)
...
>
> 无论名字是否上榜，你们的每一份支持都是 AIYXDATA-TRADAR 能够走到今天的基石。🙏
...
|           *W          |  6  | 2025.11.13  |      | 
|           *凯          |  1  | 2025.11.13  |      | 
|           对*.          |  1  | 2025.11.13  |    Thanks for your AIYXDATA-TRADAR  | 
|           s*y          |  1  | 2025.11.13  |      | 
|           **翔          |  10  | 2025.11.13  |   好项目，相见恨晚，感谢开源！     | 
|           *韦          |  9.9  | 2025.11.13  |   AIYXDATA-TRADAR超赞，请老师喝咖啡~     | 
...
|           **杰           |  10  | 2025.10.16  |          |
|           *啸           |  10  | 2025.10.16  |          |
|           *纪           |  5  | 2025.10.14  | AIYXDATA-TRADAR         |
...
> 若 AIYXDATA-TRADAR 曾为你捕捉价值，不妨为它注入动力，助其持续进化
>
...
> **📌 查看最新更新**：**[原仓库更新日志](https://github.com/AIYXDATA/AIYXDATA-TRADAR?tab=readme-ov-file#-更新日志)** ：
...
- **代码复用优化**：批次处理函数直接复用 aiyxdata_tradar 核心模块，不重复造轮子
...
- **新增 check_version 工具**：支持同时检查 AIYXDATA-TRADAR 和 MCP Server 版本更新
...
- **频率词配置增强**：新增 `[组别名]` 语法，支持 `#` 注释行，配置更清晰（感谢 [@songge8](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/752) 提出的建议）
...
- **核心代码模块化**：将主程序逻辑拆分为 aiyxdata_tradar 包的多个模块，显著提升代码可维护性。
...
- 新增独立的 MCP 服务镜像 `wantcat/aiyxdata_tradar-mcp`
...
- 通过 `manage.py` 命令控制启动/停止：`docker exec -it aiyxdata_tradar python manage.py start_webserver`
...
**🐛 Bug 修复**
- 修复 `config.yaml` 中 `ntfy_server_url` 配置不生效的问题 ([#345](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/345))
...
> 感谢 [nidaye996](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/98) 发现的体验问题
...
- 修正了邮件通知配置检查被遗漏的问题（[#88](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/88)）
...
- 正式发布官方 Docker 镜像 wantcat/aiyxdata_tradar，支持多架构
...
👉 **在线体验**：[https://AIYXDATA.github.io/AIYXDATA-TRADAR/](https://AIYXDATA.github.io/AIYXDATA-TRADAR/)
...
不再被各个平台的算法牵着走，AIYXDATA-TRADAR 会重新整理全网热搜
...
>
> **💡 提示**：启用 GitHub Pages 可获得在线访问地址，进入仓库 Settings → Pages 即可开启。[效果预览](https://AIYXDATA.github.io/AIYXDATA-TRADAR/)
>
...
> **提醒**：建议先 **[查看最新官方文档](https://github.com/AIYXDATA/AIYXDATA-TRADAR?tab=readme-ov-file)**，确保配置步骤是最新的。
...
### 1️⃣ 第一步：获取项目代码
   ...
   > - 使用 Fork 可能导致运行异常，详见 [Issue #606](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/606)
...
### 2️⃣ 第二步：设置 GitHub Secrets
   ...
   **GitHub Secret 配置（⚠️ Name 名称必须严格一致）：**
   ...
   其中方案一，由 **ziventian**发现并提供建议，在这里感谢他，默认是个人推送，也可以配置群组推送操作[#97](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/97) ，
   ...
   **支持的邮箱服务商**（自动识别 SMTP 配置）：
   ...
   >
   > **反馈说明**：
   > - 如果你使用**其他邮箱**测试成功，欢迎开 [Issues](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues) 告知，我会添加到支持列表
   > - 如果上述邮箱配置有误或无法使用，也请开 [Issues](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues) 反馈，帮助改进项目
   >
   > **特别感谢**：
   > - 感谢 [@DYZYD](https://github.com/DYZYD) 贡献天翼邮箱（189.cn）配置并完成自发自收测试 ([#291](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/291))
   > - 感谢 [@longzhenren](https://github.com/longzhenren) 贡献阿里云邮箱（aliyun.com）配置并完成测试 ([#344](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/344))
   > - 感谢 [@ACANX](https://github.com/ACANX) 贡献 Yandex 邮箱（yandex.com）配置并完成测试 ([#663](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/663))
   > - 感谢 [@Sleepy-Tianhao](https://github.com/Sleepy-Tianhao) 贡献 iCloud 邮箱（icloud.com）配置并完成测试 ([#728](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/728))
   ...
   **快速开始：**
   ...
   2. **订阅主题**（选择一个难猜的名称）：
      ```
      建议格式：aiyxdata_tradar-{你的名字缩写}-{随机数字}
   
      不能使用中文
      
      ✅ 好例子：aiyxdata_tradar-zs-8492
   ...
   **配置 AIYXDATA-TRADAR**：
   ...
   NTFY_TOPIC: aiyxdata_tradar-alerts  # 自托管可用简单名称
   ...
   **常见问题：**
   ...
   如果你选择随机的、足够长的名称（如 `aiyxdata_tradar-zs-8492-news`），暴力破解几乎不可能：
   ...
   **设置步骤：**
   ...
   3. **填写 App 信息**：
      - **App Name**：填写应用名称（如 `AIYXDATA-TRADAR` 或 `热点新闻监控`）
   ...
   1. **查看生成的 URL**：
      ...
      - 格式如：`https://hooks.slack.com/services/YOUR_WORKSPACE_ID/YOUR_CHANNEL_ID/YOUR_TOKEN`

   2. **复制 URL**：
      - 点击 URL 右侧的 **"Copy"** 按钮
      - 或手动选中 URL 并复制

   3. **配置到 AIYXDATA-TRADAR**：
...
### 3️⃣ 第三步：手动测试新闻推送
   ...
   **如何找到你的 Actions 页面**：
   ...
   - **方法二**：直接访问 `https://github.com/你的用户名/AIYXDATA-TRADAR/actions`

   **示例对比**：
   - ❌ 作者的项目：`https://github.com/AIYXDATA/AIYXDATA-TRADAR/actions`
   - ✅ 你的项目：`https://github.com/你的用户名/AIYXDATA-TRADAR/actions`

   **测试步骤**：
   1. 进入你项目的 Actions 页面
   2. 找到 **"Get Hot News"**(必须得是这个字)点进去，点击右侧的 **"Run workflow"** 按钮运行 
      - 如果看不到该字样，参照 [#109](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/109) 解决
...
### 5️⃣ 第五步：远程云存储 & 签到配置
   ...
   **GitHub Secret 配置（需添加 4 项）：**
   ...
   | Name（名称） | Secret（值）说明 |
   |-------------|-----------------|
   | `S3_BUCKET_NAME` | 存储桶名称（如 `aiyxdata_tradar-data`） |
   ...
   **详细操作步骤（获取凭据）：**
   ...
   2. **创建存储桶**：
      ...
      - 输入名称（例如 `aiyxdata_tradar-data`），点击 `创建存储桶`。
   ...
   3. **创建 API 令牌**：
      ...
      - **⚠️ 关键设置**：
        ...
        - **权限**：选择 `管理员读和写` 。
        - **指定存储桶**：为了安全，建议选择 `仅适用于指定存储桶` 并选中你的桶（如 `aiyxdata_tradar-data`）。
...
### 7️⃣ 第七步：🎉 部署成功！
   ...
   恭喜！现在你可以开始享受 AIYXDATA-TRADAR 带来的高效信息流了。
...
### 8️⃣ 第八步：进阶：选择你的 AI 助手
   ...
   AIYXDATA-TRADAR 提供了两种 AI 使用方式，满足不同需求：
...
> 💡 **快捷方式**：如果不会看源代码，可以复制他人整理好的 [平台配置汇总](https://github.com/AIYXDATA/AIYXDATA-TRADAR/issues/95)
...
AIYXDATA-TRADAR 提供两个独立的 Docker 镜像，可根据需求选择部署：

| 镜像名称 | 用途 | 说明 |
|---------|------|------|
| `wantcat/aiyxdata_tradar` | 新闻推送服务 | 定时抓取新闻、推送通知（必选） |
| `wantcat/aiyxdata_tradar-mcp` | AI 分析服务 | MCP 协议支持、AI 对话分析（可选） |

> 💡 **建议**：
> - 只需要推送功能：仅部署 `wantcat/aiyxdata_tradar` 镜像
> - 需要 AI 分析功能：同时部署两个镜像
...
3. **启动服务**:
   ...
   **选项 A：启动所有服务（推送 + AI 分析）**
   ...
   # 启动所有服务（aiyxdata_tradar + aiyxdata_tradar-mcp）
   docker compose up -d
   ```

   **选项 B：仅启动新闻推送服务**
   ```bash
   # 只启动 aiyxdata_tradar（定时抓取和推送）
   ...
   **选项 C：仅启动 MCP AI 分析服务**
   ```bash
   # 只启动 aiyxdata_tradar-mcp（提供 AI 分析接口）
   docker compose pull aiyxdata_tradar-mcp
   docker compose up -d aiyxdata_tradar-mcp
   ```

   > 💡 **提示**：
   > - 大多数用户只需启动 `aiyxdata_tradar` 即可实现新闻推送功能
   > - 只有需要使用 ChatGPT/Gemini 进行 AI 对话分析时，才需启动 `aiyxdata_tradar-mcp`
   > - 两个服务相互独立，可根据需求灵活组合
...
4. **查看运行状态**:
   ...
   # 停止特定服务
   docker compose stop aiyxdata_tradar      # 停止推送服务
   docker compose stop aiyxdata_tradar-mcp  # 停止 MCP 服务
   ```
...
| 镜像名称 | 用途 | 说明 |
|---------|------|------|
| `wantcat/aiyxdata_tradar` | 新闻推送服务 | 定时抓取新闻、推送通知 |
| `wantcat/aiyxdata_tradar-mcp` | MCP 服务 | AI 分析功能（可选） |
...
docker exec -it aiyxdata_tradar python manage.py start_webserver   # 启动 Web 服务器
docker exec -it aiyxdata_tradar python manage.py stop_webserver    # 停止 Web 服务器
docker exec -it aiyxdata_tradar python manage.py webserver_status  # 查看 Web 服务器状态
...
AIYXDATA-TRADAR 生成的当日汇总 HTML 报告会同时保存到两个位置：
...
# 查看运行状态
docker ps | grep aiyxdata_tradar-mcp
```
...
AIYXDATA-TRADAR v3.0.0 新增了基于 **MCP (Model Context Protocol)** 的 AI 分析功能，让你可以通过自然语言与新闻数据对话，进行深度分析。
...
AIYXDATA-TRADAR MCP 服务支持标准的 Model Context Protocol (MCP) 协议，可以接入各种支持 MCP 的 AI 客户端进行智能分析。

### 支持的客户端

**注意事项**：
- 将 `/path/to/AIYXDATA-TRADAR` 替换为你的项目实际路径
- Windows 路径使用双反斜杠：`C:\\Users\\YourName\\AIYXDATA-TRADAR`
- 保存后记得重启
   ...
   ```json
   {
     "mcpServers": {
       "aiyxdata_tradar": {
         ...
         "description": "AIYXDATA-TRADAR 新闻热点聚合分析"
       }
     }
   }
   ```
...
任何支持 Model Context Protocol 的客户端都可以连接 AIYXDATA-TRADAR：
...
**注意事项**：
- 替换 `/path/to/AIYXDATA-TRADAR` 为实际项目路径
...
- **项目地址**：https://github.com/AIYXDATA/ai-code-context-helper
